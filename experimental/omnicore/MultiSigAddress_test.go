...
func TestCreateMultiSigAddr(t *testing.T) {
	...
	address, redeemScriptHex, _ := CreateMultiSigAddr(addr1_pubkey, addr2_pubkey, &chaincfg.MainNetParams)
	fmt.Println(address)
	...
	address, redeemScriptHex, _ = CreateMultiSigAddr(addr1_pubkey, addr2_pubkey, &chaincfg.TestNet3Params)
	fmt.Println("expect: ")
