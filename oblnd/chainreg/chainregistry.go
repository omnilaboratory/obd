...
type Config struct {
	...
	OmmicoinproxydMode *lncfg.Bitcoind
...
func NewPartialChainControl(cfg *Config) (*PartialChainControl, func(), error) {
	...
	case "neutrino":
		...
		//neutrino mode need omniproxy-jsonrpc-api go get token info.
		...
		rpcConfig := &rpcclient.ConnConfig{
			Host:                 cfg.OmmicoinproxydMode.RPCHost,
			User:                 cfg.OmmicoinproxydMode.RPCUser,
			Pass:                 cfg.OmmicoinproxydMode.RPCPass,
		...
		cc.Rclient = chainConn

	case "omnicoreproxy", "bitcoind", "litecoind":
		...
		case cfg.Bitcoin.Active:
			...
			* obd update wxf : add
			...
			} else if homeChainConfig.Node == "omnicoreproxy" {
				bitcoindMode = cfg.OmmicoinproxydMode
			}
...
var (
	...
	ChainDNSSeedsByName = map[string][][2]string{
		"regtest": {{
			"reg.nodes.oblnd.top",
			"alina.ns.cloudflare.com",
		}},
