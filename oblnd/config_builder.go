...
func (d *DefaultWalletImpl) BuildWalletConfig(ctx context.Context,
	...
	*btcwallet.Config, func(), error) {
	...
	/*obd update wxf
	 *reset Birthday ,regtest wallet's first sync can take less time.
	 * example : regtest gen tow blcock every minute with the birthday default setting,
	 * the wallte first-sync will sync 5000 blocks,will make 10000+ times rpc-call to sync regtest blocks; it will take long time before i can make test.
	 */
	if d.cfg.ActiveNetParams.Params == &chaincfg.RegressionNetParams {
		walletInitParams.Birthday = time.Now().Add(45 * time.Hour)
	}
	...
	chainControlCfg := &chainreg.Config{
		...
		OmmicoinproxydMode:          d.cfg.OmnicoinproxydMode,
