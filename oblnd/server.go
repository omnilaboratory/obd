...
func (s *server) Start() error {
	...
	s.start.Do(func() {
		...
		if shouldPeerBootstrap(s.cfg) {
			srvrLog.Infof("Auto peer bootstrapping is start")
...
func initNetworkBootstrappers(s *server) ([]discovery.NetworkPeerBootstrapper, error) {
	...
	if !s.cfg.Bitcoin.SimNet || !s.cfg.Litecoin.SimNet {
		...
		/*
			ob add wxf
			add ChainDNSSeeds networkName
		*/
		seeds, ok := chainreg.ChainDNSSeedsByName[s.cfg.ActiveNetParams.Name]
		if ok {
			srvrLog.Infof("Creating DNS peer bootstrapper with "+
				"seeds: %v", dnsSeeds)

			dnsBootStrapper := discovery.NewDNSSeedBootstrapper(
				seeds, s.cfg.net, s.cfg.ConnectionTimeout,
			)
			bootStrappers = append(bootStrappers, dnsBootStrapper)
		}
...
func shouldPeerBootstrap(cfg *Config) bool {
	...
	/*
		obd update wxf
		enable shouldPeerBootstrap when Regtest
		enable shouldPeerBootstrap when TestNet3
	*/
	isDevNetwork := isSimnet || isSignet
