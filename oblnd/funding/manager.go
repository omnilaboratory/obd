...
func (f *Manager) handleFundingOpen(peer lnpeer.Peer,
	...
	shutdown, err := getUpfrontShutdownScript(
		...
		func() (lnwire.DeliveryAddress, error) {
			/*todo check charge adders and  Shutdown addres*/
			//addr, err := f.cfg.Wallet.NewAddress(
			//	lnwallet.PubKey, false,
			...
			/* obd update wxf
			set Shutdown addre as the wallet default address
			*/
			addr, err := f.cfg.Wallet.OB_GetDefaultAddress()
			if err != nil {
				return nil, err
			}
...
func makeFundingScript(channel *channeldb.OpenChannel) ([]byte, error) {
	...
	/*
		obd update wxf
		todo check makeFundingScript usage
	*/
	//return input.WitnessScriptHash(multiSigScript)
	return input.GenerateP2SH(multiSigScript)
}
...
func (f *Manager) handleInitFundingMsg(msg *InitFundingMsg) {
	...
	shutdown, err := getUpfrontShutdownScript(
		...
		func() (lnwire.DeliveryAddress, error) {
			...
			//)
			/* obd update wxf
			set shutdown-address to wallet default-addre
			*/
			addr, err := f.cfg.Wallet.OB_GetDefaultAddress()
			if err != nil {
				return nil, err
