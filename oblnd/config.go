...
func DefaultConfig() Config {
	return Config{
		...
		OmnicoinproxydMode: &lncfg.Bitcoind{
			Dir:                defaultBitcoindDir,
			RPCHost:            defaultRPCHost,
			EstimateMode:       defaultBitcoindEstimateMode,
			PrunedNodeMaxPeers: defaultPrunedNodeMaxPeers,
...
func ValidateConfig(cfg Config, interceptor signal.Interceptor, fileParser,
	...
	cfg.OmnicoinproxydMode.Dir = CleanAndExpandPath(cfg.OmnicoinproxydMode.Dir)
	...
	case cfg.Bitcoin.Active:
		...
		case "bitcoind":
			...
			/*
			*obd update wxf
			*add omnicoinproxyd support
			 */
		case "omnicoreproxy":
			if cfg.Bitcoin.SimNet {
				return nil, mkErr("omnicoinproxyd does not " +
					"support simnet")
			}
			//omnicoinproxyd is public,and can by accessed by any anonymous/pass
			cfg.OmnicoinproxydMode.RPCUser = "Anonymous"
			cfg.OmnicoinproxydMode.RPCPass = "public"
			err := parseRPCParams(
				cfg.Bitcoin, cfg.OmnicoinproxydMode,
				chainreg.BitcoinChain, cfg.ActiveNetParams,
			)
			if err != nil {
				return nil, mkErr("omnicoinproxyd unable to load RPC "+
					"credentials for bitcoind: %v", err)
			}
		case "neutrino":
			...
			/*obd update wxf
			neutrino need omnicoinproxy to get asset info(example usdt).
			omnicoinproxy is public,and can by accessed by any anonymous/pass
			*/
			cfg.OmnicoinproxydMode.RPCUser = "Anonymous"
			cfg.OmnicoinproxydMode.RPCPass = "public"
			if cfg.OmnicoinproxydMode.RPCHost == "" || !strings.Contains(cfg.OmnicoinproxydMode.RPCHost, ":") {
				return nil, mkErr("omnicoinproxy rpchost must set with format 'host:port'; neutrino need omnicoinproxy to get asset info")
			}
		case "nochainbackend":
...
func parseRPCParams(cConfig *lncfg.Chain, nodeConfig interface{},
	...
	case "bitcoind", "litecoind", "omnicoreproxy":
