...
func (lc *LightningChannel) evaluateHTLCView(view *htlcView, ourBtcBalance,
	...
	for _, entry := range view.theirUpdates {
		...
		if mutateState && entry.EntryType == Settle && !remoteChain &&
			...
			lc.channelState.TotalAssetSent += entry.AssetAmount
		}
...
func (lc *LightningChannel) CompleteCooperativeClose(
	...
	*/
	...
	vm, err := txscript.NewEngine(prevOut.PkScript, closeTx, 0,
		txscript.StandardVerifyFlags, nil, nil, prevOut.Value, txscript.NewCannedPrevOutputFetcher(
...
func (lc *LightningChannel) OB_AssetCompleteCooperativeClose(
	...
	if len(closeTx2.TxIn) > 0 {
		...
		vm, err := txscript.NewEngine(fundingOut.PkScript, closeTx2, 0,
			txscript.StandardVerifyFlags, nil, nil, closeTx1.TxOut[2].Value, txscript.NewCannedPrevOutputFetcher(
				fundingOut.PkScript, fundingOut.Value,
			))
		if err != nil {
			return nil, nil, 0, err
