...
var sendCoinsFromCommand = cli.Command{
	Name:     "sendcoinsfrom",
	Category: "On-chain",
	Usage:    "Send bitcoin on-chain to an address.",
	Description: `
	Send amt coins in satoshis to the base58 or bech32 encoded bitcoin address addr from special address.
	...
	Action: actionDecorator(sendCoinsFrom),
}

func sendCoinsFrom(ctx *cli.Context) error {
	var (
		addr         string
		amt          int64
		from         string
	...
	if ctx.NArg() == 0 && ctx.NumFlags() == 0 {
		cli.ShowCommandHelp(ctx, "sendcoinsfrom")
		return nil
	}
	...
	// Only fee rate flag or conf_target should be set, bo notth.
	if _, err := checkNotBothSet(
		ctx, feeRateFlag, "conf_target",
	); err != nil {
		return err
	...
	minConfs := int32(ctx.Uint64("min_confs"))
	req := &lnrpc.SendCoinsFromRequest{
		Addr:             addr,
		Amount:           amt,
		TargetConf:       int32(ctx.Int64("conf_target")),
		SatPerVbyte:      ctx.Uint64(feeRateFlag),
	...
	txid, err := client.OB_SendCoinsFrom(ctxc, req)
	if err != nil {
		return err
	}
...
var walletBalanceByAddressCommand = cli.Command{
	Name:     "walletbalancebyaddress",
	Category: "Wallet",
	Usage:    "Compute and display the  wallet's current balance by address.",
	Action:   actionDecorator(walletBalanceByAddress),
	Flags: []cli.Flag{
		cli.StringFlag{
			Name:  "address",
			Usage: "the address used for balance query",
		},
		//cli.IntFlag{
		//	Name:  "asset_id",
		//	Usage: "asset_id ",
...
func walletBalanceByAddress(ctx *cli.Context) error {
	ctxc := getContext()
	client, cleanUp := getClient(ctx)
	defer cleanUp()

	req := &lnrpc.WalletBalanceByAddressRequest{Address: ctx.String("address")}
	resp, err := client.OB_WalletBalanceByAddress(ctxc, req)
	if err != nil {
		return err
	}
...
var assetsBalanceByAddressCommand = cli.Command{
	Name:     "assetsbalancebyaddress",
	Category: "Wallet",
	Usage:    "Compute and display the   assets balance by address.",
	Action:   actionDecorator(assetsBalanceByAddress),
	Flags: []cli.Flag{
		cli.StringFlag{
			Name:  "address",
			Usage: "the address used for balance query",
...
func assetsBalanceByAddress(ctx *cli.Context) error {
	ctxc := getContext()
	client, cleanUp := getClient(ctx)
	defer cleanUp()

	req := &lnrpc.AssetsBalanceByAddressRequest{Address: ctx.String("address")}
	resp, err := client.OB_AssetsBalanceByAddress(ctxc, req)
	if err != nil {
		return err
	}
	printRespJSON(resp)
...
var assetInfoCommand = cli.Command{
	Name:     "assetinfo",
	Category: "Wallet",
	Usage:    "get assetinfo ,map omnicore-rpc omni_getproperty",
	Action:   actionDecorator(assetInfo),
	Flags: []cli.Flag{
		cli.Int64Flag{
			Name:  "asset_id",
			Usage: "",
...
func assetInfo(ctx *cli.Context) error {
	ctxc := getContext()
	client, cleanUp := getClient(ctx)
	defer cleanUp()

	req := &lnrpc.GetAssetInfoRequest{AssetId: ctx.Int64("asset_id")}
	resp, err := client.OB_GetAssetInfo(ctxc, req)
	if err != nil {
		return err
	}
	printRespJSON(resp)
...
func channelBalance(ctx *cli.Context) error {
	...
	client, cleanUp := getClient(ctx)
	defer cleanUp()
	asset_id:= uint32(ctx.Int("asset_id"))
	req := &lnrpc.ChannelBalanceRequest{AssetId: asset_id}
...
func getInfo(ctx *cli.Context) error {
	...
	resp, err := client.OB_GetInfo(ctxc, req)
	if err != nil {
		return err
	}
...
func getNetworkInfo(ctx *cli.Context) error {
	...
	asset_id:= uint32(ctx.Int("asset_id"))
	req := &lnrpc.NetworkInfoRequest{AssetId: asset_id}
...
var listAssetTxsCommand = cli.Command{
	Name:     "listassetstxs",
	Category: "On-chain",
	Usage:    "List transactions for asset.",
	Flags: []cli.Flag{
		cli.StringFlag{
			Name: "address",
			Usage: "the block height from which to list " +
	...
	Description: `List all asset transactions for address.`,
	Action:      actionDecorator(listassetTxns),
}

func listassetTxns(ctx *cli.Context) error {
	ctxc := getContext()
	client, cleanUp := getClient(ctx)
	defer cleanUp()

	address := ctx.String("address")
	req := &lnrpc.ListTranscationsRequest{}
	req.Addrs = append(req.Addrs, address)
	resp, err := client.OB_ListTranscations(ctxc, req)
	if err != nil {
		return err
	}
...
var getOmniTxCommand = cli.Command{
	Name:     "omni_gettransaction",
	Category: "On-chain",
	Usage:    "Get detailed information about an Omni transaction.",
	Flags: []cli.Flag{
		cli.StringFlag{
			Name:  "txid",
			Usage: "the hex-encoded hash of the transaction",
		},
	},
	Description: `Get detailed information about an Omni transaction.`,
	Action:      actionDecorator(getOmniTx),
}

func getOmniTx(ctx *cli.Context) error {
	ctxc := getContext()
	client, cleanUp := getClient(ctx)
	defer cleanUp()

	txid := ctx.String("txid")
	req := &lnrpc.GetOmniTransactionRequest{Txid: txid}
	resp, err := client.OB_GetOmniTransaction(ctxc, req)
	if err != nil {
		return err
	}
