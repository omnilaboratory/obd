...
func trackPayment(ctx *cli.Context) error {
	...
	stream, err := routerClient.OB_TrackPaymentV2(ctxc, req)
	if err != nil {
		return err
	}
...
func printLivePayment(ctxc context.Context,
	stream routerrpc.Router_OB_TrackPaymentV2Client,
...
func formatPaymentMsat(amt int64) string {
...
func formatPayment(ctxc context.Context, payment *lnrpc.Payment,
	...
	for _, htlc := range payment.Htlcs {
		...
		t.AppendRow([]interface{}{
			...
			formatPaymentMsat(lastHop.AmtToForwardMsat),
			formatPaymentMsat(route.TotalFeesMsat),
	...
	fmt.Fprintf(b, "Amount + fee:   %v + %v minUnit\n",
		formatPaymentMsat(totalPaid), formatPaymentMsat(totalFees))
