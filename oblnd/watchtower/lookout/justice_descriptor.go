...
func (s * breachedInput)GetSigScript()([]byte,error){
	if len(s.witness)==0{return nil,nil}
...
func (p *JusticeDescriptor) commitToLocalInput() (*breachedInput, error) {
	...
	toLocalWitnessHash, err := input.GenerateP2SH(toLocalScript)
	if err != nil {
		return nil, err
	}
...
func (p *JusticeDescriptor) commitToRemoteInput() (*breachedInput, error) {
	...
	if p.JusticeKit.BlobType.IsAnchorChannel() {
		...
		//	toRemoteScript,
		//)
		toRemoteScriptHash, err = input.GenerateP2SH(
			toRemoteScript,
		)
		if err != nil {
			return nil, err
		...
		//	toRemotePubKey,
		//)
		toRemoteScriptHash, err =input.GenerateP2PKH(toRemotePubKey.SerializeCompressed())
		if err != nil {
			return nil, err
		}
	}
...
func (p *JusticeDescriptor) assembleJusticeTxn(txWeight int64,
	...
	for _, input := range inputs {
		...
		justiceTxn.TxIn[i].SignatureScript,err = input.GetSigScript()
		if err != nil {
			return nil,err
		}
