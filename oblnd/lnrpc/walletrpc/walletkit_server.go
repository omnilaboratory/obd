...
func (w *WalletKit) ListUnspent(ctx context.Context,
	...
	err = w.cfg.CoinSelectionLocker.WithCoinSelectLock(func() error {
		utxos, err = w.cfg.Wallet.ListUnspentP2pkh(
...
func (w *WalletKit) NextAddr(ctx context.Context,
	...
	//		"address: %v", req.Type)
	//}
	if req.Type!=AddressType_PubKeyHash{
		return nil, fmt.Errorf("omni only support PubKeyHash")
	}

	addr, err := w.cfg.Wallet.OB_NewAddress(addrType, req.Change, account)
	if err != nil {
		return nil, err
	}
...
func (w *WalletKit) BumpFee(ctx context.Context,
	...
	/*obd update wxf
	omni not support witeness
	*/
	case lnwallet.PubKey:
		witnessType = input.PubKeyHash
...
func marshalWalletAccount(internalScope waddrmgr.KeyScope,
	...
	case waddrmgr.KeyScopeBIP0044:
		addrType = AddressType_PubKeyHash
