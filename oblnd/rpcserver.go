...
func (r *rpcServer) OB_ListAsset(context.Context, *lnrpc.ListAssetRequest) (*lnrpc.ListAssetResponse, error) {
	bs, err := r.server.cc.Rclient.RawRequest("omni_listproperties", nil)
	if err != nil {
		return nil, err
	}
	var items []*lnrpc.Asset
	...
	return &lnrpc.ListAssetResponse{List: items1}, nil
}
...
func (r *rpcServer) OB_SendCoinsFrom(ctx context.Context,
...
func (r *rpcServer) ListUnspent(ctx context.Context,
	...
	err = r.server.cc.Wallet.WithCoinSelectLock(func() error {
		utxos, err = r.server.cc.Wallet.ListUnspentP2pkh(
...
func (r *rpcServer) SendCoins(ctx context.Context,
	...
	if in.SendAll {
		...
		if err == lnwallet.ErrReservedValueInvalidated {
			...
			changeAddr, err := r.server.cc.Wallet.OB_NewAddress(
				lnwallet.PubKey, true,
...
func (r *rpcServer) OB_ListAddresses(ctx context.Context, in *lnrpc.ListAddressesRequest) (*lnrpc.ListAddressesResponse, error) {
	// obd only use AddressType_PUBKEY and DefaultAccountName
...
func (r *rpcServer) OB_ListRecAddress(ctx context.Context, in *lnrpc.ListRecAddressRequest) (*lnrpc.ListRecAddressResponse, error) {
	res, err := r.server.cc.Wallet.OB_ListRecAddress()

	items := []*lnrpc.RecAddress{}
	for _, item := range res {
		items = append(items, &lnrpc.RecAddress{Addre: item.Addre, IsDefault: item.IsDefault})
	}
	return &lnrpc.ListRecAddressResponse{Items: items}, err
}
...
func (r *rpcServer) OB_NewAddress(ctx context.Context,
	...
	/*bod update wxf
	* record the user create address
	 */
	err = r.server.cc.Wallet.RecordAddress(addr.String())
	rpcsLog.Debugf("record user address %s %v", addr, err)
...
func (r *rpcServer) parseOpenChannelReq(in *lnrpc.OpenChannelRequest,
	...
	if remoteInitialBtcBalance >= localFundingBtcAmt {
		return nil, fmt.Errorf("amount pushed to remote peer for "+
			"initial state must be below the local funding amount: remote %v ,local %v", remoteInitialBtcBalance, localFundingBtcAmt)
	}
	if in.AssetId > lnwire.BtcAssetId && remoteInitialAssetBalance >= localFundingAssetAmt {
		return nil, fmt.Errorf("amount pushed to remote peer for "+
...
// OB_OpenChannel attempts to open a singly funded channel specified in the
// request to a remote peer.
func (r *rpcServer) OB_OpenChannel(in *lnrpc.OpenChannelRequest,
	updateStream lnrpc.Lightning_OB_OpenChannelServer) error {
...
func (r *rpcServer) CloseChannel(in *lnrpc.CloseChannelRequest,
	...
	if force {
		...
		/*
			obd update wxf
			asset-Tx's TxOut[0] is op_return output,not a pkscript ,can't used in ChainNotifier
		*/
		pkScript := closingTx.TxOut[0].PkScript
		if bytes.HasPrefix(pkScript, []byte{txscript.OP_RETURN}) {
			pkScript = closingTx.TxOut[1].PkScript
		}

		errChan = make(chan error, 1)
		notifier := r.server.cc.ChainNotifier
		go peer.WaitForChanToClose(uint32(bestHeight), notifier, errChan, chanPoint,
			&closingTxid, pkScript, func() {
...
// OB_GetInfo returns general information concerning the lightning node including
// its identity pubkey, alias, the chains it is connected to, and information
// concerning the number of open+pending channels.
func (r *rpcServer) OB_GetInfo(_ context.Context,
	...
	/*
		obd upadte wxf:
		PubkeyBech32 can be used in dns servicey srv record
	*/
	rawkey := r.server.identityECDH.PubKey().SerializeCompressed()
...
//omni_getproperty
func (r *rpcServer) OB_GetAssetInfo(ctx context.Context, in *lnrpc.GetAssetInfoRequest) (*lnrpc.GetAssetInfoResponse, error) {
	params := []json.RawMessage{}
	addressJson, err := json.Marshal(in.AssetId)
	if err != nil {
		return nil, err
	}
	params = append(params, json.RawMessage(addressJson))
	bs, err := r.server.cc.Rclient.RawRequest("omni_getproperty", params)
	if err != nil {
		return nil, err
	}
	assetRes := &lnrpc.GetAssetInfoResponse{}
	err = json.Unmarshal(bs, assetRes)
	return assetRes, nil
}
...
func (r *rpcServer) OB_AssetsBalanceByAddress(ctx context.Context, in *lnrpc.AssetsBalanceByAddressRequest) (*lnrpc.AssetsBalanceByAddressResponse, error) {
	//query assetBalance
	params := []json.RawMessage{}
	addressJson, err := json.Marshal(in.Address)
	if err != nil {
		return nil, err
	}
	params = append(params, json.RawMessage(addressJson))
	bs, err := r.server.cc.Rclient.RawRequest("omni_getallbalancesforaddress", params)
	if err != nil {
		return nil, err
	}
	assetsBRes := []*assetBalance{}
	err = json.Unmarshal(bs, &assetsBRes)
	if err != nil {
		return nil, err
	}
	list := []*lnrpc.AssetBalanceByAddressResponse{}
	for _, ab := range assetsBRes {
		assetBalance, err := strconv.ParseFloat(ab.Balance, 64)
		if err != nil {
			return nil, err
		}
		aBlance, _ := omnicore.NewAmount(assetBalance)
		a := &lnrpc.AssetBalanceByAddressResponse{
			Propertyid: ab.Propertyid,
			Name:       ab.Name,
			Balance:    int64(aBlance),
			Reserved:   ab.Reserved,
			Frozen:     ab.Frozen,
		}
		a.Propertyid = ab.Propertyid
		list = append(list, a)
	}
	return &lnrpc.AssetsBalanceByAddressResponse{List: list}, nil
}
 ...
 */
func (r *rpcServer) OB_WalletBalanceByAddress(ctx context.Context,
	in *lnrpc.WalletBalanceByAddressRequest) (*lnrpc.WalletBalanceByAddressResponse, error) {
	...
	for _, account := range accounts {
		...
		// Get total balance, from txs that have >= 0 confirmations.
		totalBal, err := r.server.cc.Wallet.ConfirmedBalanceByAddress(
			0, in.Address, account.AccountName,
		)
		if err != nil {
			return nil, err
		}
		...
		// TODO(halseth): get both unconfirmed and confirmed balance in
		// one call, as this is racy.
		confirmedBal, err := r.server.cc.Wallet.ConfirmedBalanceByAddress(
			1, in.Address, account.AccountName,
		)
		if err != nil {
			return nil, err
		}
	...
	rpcsLog.Debugf("[WalletBalanceByAddress] addres=%v Total balance=%v (confirmed=%v, "+
		"unconfirmed=%v)", in.Address, totalBalance, confirmedBalance,
		unconfirmedBalance)

	//query assetBalance
	//params := []json.RawMessage{}
	//addressJson, _ := json.Marshal(in.Address)
	//params = append(params, json.RawMessage(addressJson))
	//assetIdJson, err := json.Marshal(in.AssetId)
	//if err != nil {
	//	return nil, err
	//}
	//params = append(params, assetIdJson)
	//bs, err := r.server.cc.Rclient.RawRequest("omni_getbalance", params)
	//if err != nil {
	//	return nil, err
	//}
	//assetBRes := new(assetBlanceRes)
	//err = json.Unmarshal(bs, assetBRes)
	//if err != nil {
	//	return nil, err
	//}
	//assetBalance, err := strconv.ParseFloat(assetBRes.Balance, 64)
	//if err != nil {
	//	return nil, err
	//}
	//aBlance, _ := omnicore.NewAmount(assetBalance)

	return &lnrpc.WalletBalanceByAddressResponse{
		Address:            in.Address,
		TotalBalance:       int64(totalBalance),
		ConfirmedBalance:   int64(confirmedBalance),
		UnconfirmedBalance: int64(unconfirmedBalance),
...
type assetBlanceRes struct {
	Balance  string `json:"balance" example:"0.00000000"`
	Frozen   string `json:"frozen" example:"0.00000000"`
	Reserved string `json:"reserved" example:"0.00000000"`
}
...
func (r *rpcServer) ChannelBalance(ctx context.Context,
	...
	*lnrpc.ChannelBalanceResponse, error) {
	...
	//if in.AssetId == 0 && in.AssetId != lnwire.BtcAssetId {
	//	return nil, errors.New("miss asset_id")
	//}
	var (
...
func (r *rpcServer) OB_ListTranscations(ctx context.Context,
	in *lnrpc.ListTranscationsRequest) (*lnrpc.ListTranscationsResponse, error) {
	...
	if len(in.Addrs) == 0 {
		...
		rpcsLog.Debugf("[ListTranscations] fetched %v channels from DB",
			len(dbChannels))
		...
		for _, dbChannel := range dbChannels {
			...
			closeAddress := addresses[0].String()
			log.Println(closeAddress)
			addrs = append(addrs, []byte(closeAddress))
		}
	} else {
	...
	for _, addr := range addrs {
		address, err := json.Marshal(string(addr))
		if err != nil {
			return nil, err
		}
		bs, err := r.server.cc.Rclient.RawRequest("omni_listtransactions", []json.RawMessage{address, []byte("1000")})
		if err != nil {
			return nil, err
		}
		var items []*lnrpc.AssetTx
		err = json.Unmarshal(bs, &items)
		if err != nil {
			return nil, err
		}
	...
	return &lnrpc.ListTranscationsResponse{List: txs}, nil
}
func (r *rpcServer) OB_GetOmniTransaction(ctx context.Context,
	in *lnrpc.GetOmniTransactionRequest) (*lnrpc.AssetTx, error) {

	if in.Txid == "" {
		return nil, errors.New("miss Txid ")
	}
	address, err := json.Marshal(string(in.Txid))
	if err != nil {
		return nil, err
	}
	bs, err := r.server.cc.Rclient.RawRequest("omni_gettransaction", []json.RawMessage{address})
	if err != nil {
		return nil, err
	}
	var tx lnrpc.AssetTx
...
// OB_AddInvoice attempts to add a new invoice to the invoice database. Any
// duplicated invoices are rejected, therefore all invoices *must* have a
// unique payment preimage.
func (r *rpcServer) OB_AddInvoice(ctx context.Context,
	...
	//	return nil, err
	//}
	value := lnwire.UnitPrec11(0)
	//if invoice.AssetId == 0 {
	//	return nil, errors.New("miss assetId")
	//}
	if invoice.AssetId == lnwire.BtcAssetId {
		value = lnwire.UnitPrec11(invoice.ValueMsat)
	} else {
		value = lnwire.UnitPrec11(invoice.Amount)
	}
...
// OB_ListInvoices returns a list of all the invoices currently stored within the
// database. Any active debug invoices are ignored.
func (r *rpcServer) OB_ListInvoices(ctx context.Context,
...
func (r *rpcServer) GetNetworkInfo(ctx context.Context,
	...
	var (
		...
		numChannels          uint32
		maxChanOut           uint32
		totalNetworkCapacity lnwire.UnitPrec8
		minChannelSize       lnwire.UnitPrec8 = math.MaxInt64
		maxChannelSize       lnwire.UnitPrec8
		medianChanSize       lnwire.UnitPrec8
	)
...
// OB_ListPayments returns a list of outgoing payments determined by a paginated
// database query.
func (r *rpcServer) OB_ListPayments(ctx context.Context,
...
func (r *rpcServer) DecodePayReq(ctx context.Context,
	...
	var amtMsat int64
	var assetAmt int64
	if payReq.AssetId != nil && *payReq.AssetId == lnwire.BtcAssetId {
		amtMsat = int64(*payReq.MilliSat)
	} else {
		assetAmt = int64(*payReq.MilliSat)
	}
